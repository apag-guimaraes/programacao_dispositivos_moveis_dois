    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions,
        ...
        if (requestCode == CODIGO_SOLICITACAO) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                // O usuário recusou uma vez
                boolean mostrarExplicacao = ActivityCompat.shouldShowRequestPermissionRationale(this, PERMISSAO);
                if (mostrarExplicacao) {
                    // Caso 1: O usuário apenas clicou em "Negar". 
                    // Ele ainda pode receber o popup do sistema de novo no futuro.
                    // Mostramos o seu dialog explicando o motivo.
                    PermissaoDialogFragment dialog = new PermissaoDialogFragment();
                    dialog.show(getSupportFragmentManager(), "PermissaoDialog");
                } else {
                    finish(); //ou finishAffinity() ou finishAndRemoveTask(); 
                }
            }
        }
    }
...
- PermissaoDialogFragment.kt
[source,kotlin]
import android.app.Dialog
import android.os.Bundle
import androidx.appcompat.app.AlertDialog
import androidx.fragment.app.DialogFragment
class PermissaoDialogFragment : DialogFragment() {
...
class MainActivity : AppCompatActivity() {
    ...
    companion object {
        private const val CODIGO_SOLICITACAO = 1
        private const val PERMISSAO = Manifest.permission.CAMERA
    }
    ...
    private fun solicitarPermissao() {
        val temPermissao = ContextCompat.checkSelfPermission(this, PERMISSAO)
        if (temPermissao != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(this, arrayOf(PERMISSAO), CODIGO_SOLICITACAO)
        } else {
            chamarActivity()
        }
    }
    private fun chamarActivity() {
        // Sintaxe concisa do Kotlin para Intents (SegundaActivity::class.java)
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == CODIGO_SOLICITACAO) {
            if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                val mostrarExplicacao = ActivityCompat.shouldShowRequestPermissionRationale(this, PERMISSAO)
                if (mostrarExplicacao) {
                    val dialog = PermissaoDialogFragment()
                    dialog.show(getSupportFragmentManager(), "PermissaoDialog")
                } else {
                    finishAndRemoveTask()
                }
...
class SegundaActivity : AppCompatActivity() {
    ...
    private val cameraLauncher = registerForActivityResult(
        ...
        if (result.resultCode == RESULT_OK && result.data != null) {
            ...
            val foto: Bitmap? = if (Build.VERSION.SDK_INT >= 33) {
    ...
    public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
        return new AlertDialog.Builder(requireContext())
                ...
                .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        dialog.dismiss(); // Apenas fecha o alerta para o usuário 
                    }
                })
                .create();
    }
    ...
    public void onRequestPermissionsResult(int requestCode,
        ...
        if (requestCode == REQUEST_PERMISSION) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                // CORRIGIDO: Todo este bloco agora está adequadamente dentro do 'else'
                // O usuário recusou a permissão.
                boolean mostrarExplicacao = ActivityCompat.shouldShowRequestPermissionRationale(this, PERMISSAO);
                if (mostrarExplicacao) {
                    // Caso 1: O usuário apenas clicou em "Negar" na primeira vez. 
                    // Mudado o nome para bater exatamente com a classe abaixo:
                    PermissionDialogFragment dialog = new PermissionDialogFragment();
                    dialog.show(getSupportFragmentManager(), "PermissaoDialog");
                } else {
                    // Caso 2: Negou permanentemente pela segunda vez.
                    finishAndRemoveTask(); 
                }
            }
        }
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        return AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton("OK") { dialog, _ ->
                dialog.dismiss() // Apenas fecha o alerta para o usuário tentar clicar em ligar de novo
            }
            .create()
    }
}
...
import androidx.core.app.ActivityCompat
import androidx.core.content.ContextCompat
import com.example.seuprojeto.databinding.ActivityMainBinding // Substitua pelo pacote correto do seu app
class MainActivity : AppCompatActivity() {
    private lateinit var binding: ActivityMainBinding
    companion object {
        private const val PERMISSAO = Manifest.permission.CALL_PHONE
        private const val REQUEST_PERMISSION = 100
        private const val TELEFONE_FIXO = "tel:+55DDNUMBER"
    }
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityMainBinding.inflate(layoutInflater)
    ...
    private void verificarPermissao() {
        val granted = ContextCompat.checkSelfPermission(this, PERMISSAO)
        if (granted == PackageManager.PERMISSION_GRANTED) {
            fazerLigacao()
        } else {
    ...
    private void fazerLigacao() {
        try {
            val intent = Intent(Intent.ACTION_CALL, Uri.parse(TELEFONE_FIXO))
            startActivity(intent)
        ...
        if (requestCode == REQUEST_PERMISSION) {
            if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                if (mostrarExplicacao) {
                    ...
                    dialog.show(supportFragmentManager, "PermissaoDialog")
                } else {
                    // Caso 2: Negou permanentemente (Não perguntar novamente)
                    finishAndRemoveTask()
                }
            }
        }
