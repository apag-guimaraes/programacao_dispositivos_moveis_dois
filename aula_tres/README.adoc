...
public class Repositorio {
    ...
    public void obterDados(Callback c){
 	...
 	public void shutdown() {
        if (executor != null) {
            executor.shutdownNow(); // Cancela tarefas pendentes ao fechar a tela
        }
    }
}//class
...
public class MainActivity extends AppCompatActivity {
    ...
    private void carregarDados() {
        ...
        repositorio.obterDados(new Repositorio.Callback() {
            ...
            public void sucesso(List<String> itens) {
                if (!isFinishing()) { // Verifica se a tela ainda existe
                    adapter.clear();
    ...
    protected void onDestroy() {
        super.onDestroy();
        if (repositorio != null) {
            repositorio.shutdown(); // Evita o vazamento de memória da Thread 
        }
    }
}
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Evita chamadas duplicadas ao rotacionar a tela: só carrega se a lista atual estiver vazia
        if (viewModel.getLista().getValue() == null) {
            viewModel.carregarDados();
        }
    }
}

- Referências: As classes Conexao.java e Conversa.java foram adaptadas do site Stack Overflow (https://stackoverflow.com/questions/58889465/json-parsing-error-value-jsonstr-of-type-java-lang-string-cannot-be-converted-t) realizada pelo usuário Saad ( https://stackoverflow.com/users/7741722/saad) e respondida por Mike M. (https://stackoverflow.com/users/2850651/mike-m). Acesso em: 21 de ago. 2025.
...
class Conversao {
    fun converter(inputStream: InputStream?): String? {
        ...
        try {
            var conteudo: String?
            while (bufferedReader.readLine().also { conteudo = it } != null) {
                stringBuilder.append(conteudo).append("\n")
            }
        } catch (e: IOException) {
            e.printStackTrace()
...
class Repositorio {
    ...
    fun obterDados(c: Callback) {
        executor.execute {
            try {
                ...
                val dados: List<Item>? = gson.fromJson(jsonString, type)
    ...
    companion object {
        private const val URL = "https://jsonplaceholder.typicode.com/posts"
    }
}
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.lista.observe(this) { listaDeItens ->
            listaDeItens?.let {
                adapter.clear()
                adapter.addAll(it)
                adapter.notifyDataSetChanged()
                Toast.makeText(this, "Carregados: ${it.size}", Toast.LENGTH_SHORT).show()
            }
        }
        viewModel.mensagem.observe(this) { erro ->
            erro?.let {
...
Neste exemplo, será usado uma biblioteca que permite criar uma API REST falsa destinada à testes. Para maiores detalhes sobre o biblioteca, acesse o endereço: https://github.com/typicode/json-server. Acesso em: 30 de jan. 2023. No Android, por questões de segurança, atualmente aceita apenas comunicação via HTTPS. A equipe que desenvolveu a biblioteca também disponibiliza um servidor com HTTPS chamado My JSON Server cujo o endereço para acesso é: https://my-json-server.typicode.com/.
...
Outra opção é usar diretamente a API do GitHub, basta criar um repositório público, adicionar o arquivo db.json, clicar no botão RAW e copiar a URL gerada.

Para os projetos das aulas vamos usar as seguintes camadas: VIEW -> VIEWMODEL -> USECASE (opcional) -> REPOSITORY. Para mais detalhes sobre arquitetura de app, segue o material: GOOGLE DEVELOPERS, Guia para a arquitetura do app, disponível em: https://developer.android.com/topic/architecture?hl=pt-br. Acessado em: 26 de fev. 2026. 
